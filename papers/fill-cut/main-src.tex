...
\begin{abstract}
  ...
  and a full tensor model of Melli\`es' and Tabareau's tensorial
  logic.
\end{abstract}
...
At this point we show that any symmetric monoidal closed category with
...
\subsection{A Negative Translation of Linear Logic into FILL using Tensorial Negation}
...
negative translation of linear logic into FILL where implication plays
a central role.  Melli\`es and Tabareau give a negative translation of
the multiplicative fragment of linear logic into tensorial logic
...
However, in FILL we have par, and implication, and so we extend their
translation into one that better fits FILL.

Multi-conclusion linear logic is defined in Figure~\ref{fig:LL}.
\begin{figure}
  \begin{center}
    \begin{mathpar}
      \FILLdruleLLXXAx{} \and
  ...
  \label{fig:LL}
\end{figure}
The following is a derivation of the law-of-excluded middle, and thus
we can see that this formulation of linear logic is indeed classical:
\begin{center}
  \begin{math}
    $$\mprset{flushleft}
    \inferrule* [right=def] {
  ...
  \end{math}
\end{center}
We give a negative translation of this formulation of linear logic
into FILL as follows:
\begin{center}
  \begin{math}
    \begin{array}{lll}
      [[(I)N]] & = & [[_|_]]\\
  ...
  \end{math}
\end{center}
This translation can now be extended to sequents by translating a
linear logic sequent $[[A1,...,Ai |- B1,...,Bj]]$ into a FILL sequent
$[[x1 : (B1)N,...,xj : (Bj)N |- t1 : (A1)N,...,ti : (Ai)N]]$ for some
unique fresh variables $[[x1]],\ldots,[[xj]]$ and some terms
$[[t1]],\ldots,[[ti]]$.  We can view translations such as these as a
syntactic functor taking linear logic into its opposite.  Finally, we
...
\begin{lemma}[Negative Translation]
  ...
  $[[t1]],\ldots,[[ti]]$
  such that
  $[[x1 : (B1)N,...,xj : (Bj)N |- t1 : (A1)N,...,ti : (Ai)N]]$
  is derivable.
\end{lemma}
\begin{proof}
  This is a proof by induction on the form of the assumed sequent
  $[[A1,...,Ai |- B1,...,Bj]]$.  Throughout the proof if $[[GL]] =
  [[A1,...,Ai]]$, then $[[(GL)N]] = [[x1 : A1,...,xi : Ai]]$ for some
  unique fresh variables $[[x1]],\ldots,[[xi]]$.  Similarly, if
  $[[LL]] = [[B1,...,Bi]]$, then $[[(LL)N]] = [[t1 : B1,...,ti : Bi]]$
  for some terms $[[t1]],\ldots,[[ti]]$ that are known to exist.  We
  will use this notation when applying the inductive hypothesis to
  make the presentation easier to read.
  ...
  \item[] Case.\\ 
    \begin{center}
      ...
      \end{math}
    \end{center}
    It suffices to derive the sequent $[[x : (A)N |- t : (A)N]]$, for
    some term $[[t]]$.  Choose $[[t]] = [[x]]$, and this sequent is
    derivable by the axiom rule.
  ...
  \item[] Case.\\ 
    \begin{center}
      ...
      \end{math}
    \end{center}
    By the induction hypothesis we know $[[x : A,(LL1)N |- (GL1)N]]$ and
    $[[(LL2)N |- (GL2)N, t : A]]$ for some term $[[t]]$.  We can now
    derive $[[(LL2)N,(LL1)N |- (GL2)N,[t/x](GL1)N]]$ which is a valid
    translation of $[[GL1, GL2 |- LL1,LL2]]$.
  ...
  \item[] Case.\\ 
    \begin{center}
      ...
      \end{math}
    \end{center}
    First, note that $[[(I)N]] = [[_|_]]$.  Then this case follows by
    first applying the induction hypothesis, and then applying the
    rule $\ifrName{Pr}$.
  ...
  \item[] Case.\\ 
    \begin{center}
      ...
      \end{math}
    \end{center}
    If suffice to derive the sequent $[[x : _|_ |- .]]$, but this
    holds by simply applying the rule $\ifrName{Pl}$.
  ...
  \item[] Case.\\ 
    \begin{center}
      ...
      \end{math}
    \end{center}
    First, $[[(A (x) B)N]] = [[(A)N (+) (B)N]]$, and it suffices to
    derive $[[(LL)N |- (GL)N,t : h((A)N (+) (B)N)]]$, for some term
    $[[t]]$, but this follows from first applying the induction
    hypothesis to the premise, and then applying the rule
    $\ifrName{Parr}$.
  ...
  \item[] Case.\\ 
    \begin{center}
      ...
      \end{math}
    \end{center}
    This is follows the same pattern as the previous rule.  Simply
    apply the induction hypothesis to both premises, and then the rule
    $\ifrName{Parl}$.
  ...
  \item[] Case.\\ 
    \begin{center}
      ...
      \end{math}
    \end{center}
    It suffices to derive $[[. |- t : _|_]]$ for some term $[[t]]$,
    but this follows by applying the rule $\ifrName{Tr}$.
  ...
  \item[] Case.\\ 
    \begin{center}
      ...
      \end{math}
    \end{center}
    This case follows by first applying the induction hypothesis to
    the premise, and then applying the rule $\ifrName{Tl}$.
    ...
    \begin{center}
      \begin{math}
        ...
        \inferrule* [right=\ifrName{Impl}] {
          [[GL1 |- A , LL1 && GL2, B |- LL2]]
        }{[[GL1, A -o B, GL2 |- LL1 , LL2]]}
      \end{math}
    \end{center}
    By the induction hypothesis we know $[[x : (A)N,(LL1)N |- (GL1)N]]$
    and $[[(LL2)N |- (GL2)N,t : B]]$ for some term $[[t]]$.  Then it
    suffices to derive
    $[[(LL1)N,(LL2)N |- (GL1)N,t' : (A -o B)N,(GL2)N]]$ for some term
    $t'$. By definition $[[(A -o B)N]] = [[~(~(A)N -o ~(B)N)]]$.
    \begin{center}
      \begin{math}
        $$\mprset{flushleft}
        \inferrule* [right=] {
          ...
          \inferrule* [right=] {
            ...
            }{[[(LL1)N,(LL2)N,z : ~(A)N -o ~(B)N |- (GL1)N,(GL2)N]]}
          }{[[(LL1)N,(LL2)N,z : ~(A)N -o ~(B)N |- (GL1)N,o : _|_,(GL2)N]]}
        }{[[(LL1)N,(LL2)N |- (GL1)N,\z.o : (~(A)N -o ~(B)N) -o _|_,(GL2)N]]}
      \end{math}
    \end{center}
