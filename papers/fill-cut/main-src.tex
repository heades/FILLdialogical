...
\begin{proof}
  ...
  \begin{itemize}
  \item Take as objects $(U , (U \Rightarrow X^*), \alpha_!)$ when $(U
    , X , \alpha)$ is an object of $\dial$.  Define $(u,f) \in \alpha_!$
    iff $(u,x_1) \in \alpha \land \cdots \land (u,x_n) \in \alpha$ where
    $f(u) = (x_1,\ldots,x_n)$.

  \item Take as morphisms
    $(f , \lambda g.F^*(g(f(u)))) : (U , (U \Rightarrow X^*), \alpha_!) \to (V , (V \Rightarrow Y^*), \beta_!)$
    when $(f , F) : (U , X , \alpha) \to (V , Y , \beta)$ is a morphism in $\dial$.
  \end{itemize}
  Now we show that $\dial_!$ is cartesian.
  \begin{itemize}
  \item Suppose $A = (U , U \Rightarrow X^* , \alpha_!)$ and $B = (V ,
    V \Rightarrow Y^* , \beta_!)$ are objects of $\dial_!$. Then we
    define $A \times B = (U \times V, (V \Rightarrow (U \Rightarrow X^*)) \times (U \Rightarrow (V \Rightarrow Y^*)), \alpha_! \otimes \beta_!)$,
    where $\alpha_! \otimes \beta_!$ is defined as it is
    for the tensor product in the proof of
    Lemma~\ref{lemma:ddial_is_a_linear_category}.  Now notice that
    $(U \times V) \to (X \times Y)^*$ is isomorphic to
    $(V \Rightarrow (U \Rightarrow X^*)) \times (U \Rightarrow (V \Rightarrow Y^*))$
    and thus we have an object in $\dial_!$.

  \item Suppose $A = (U , U \Rightarrow X^* , \alpha_!)$, $B = (V , V
    \Rightarrow Y^* , \beta_!)$, and $C = (W , W \Rightarrow Z^* ,
    \gamma_!)$ are objects of $\dial_!$.  Then the we define the following
    morphisms:
    \begin{itemize}
    \item Define the first projection by $(\pi_1, \lambda f.(\lambda
      v.\lambda u.f(u), \lambda u.\lambda v.())) : A \times B \to A$,
      where $\pi_1$ is the first projection in $\sets$.  The condition
      on morphisms easily follows from knowing $\alpha_!  \otimes
      \beta_!$.  See the formalization for the details.

    \item Similarly to the first projection we define the second
      projection by $(\pi_2, \lambda f.(\lambda v.\lambda u.(),
      \lambda u.\lambda v.f(v))) : A \times B \to B$, where $\pi_2$ is
      the first projection in $\sets$.

    \item Suppose $j_1 = (f , F) : C \to A$ and $j_2 = (g , G) : C \to
      B$ are morphisms in $\dial_!$.  Then we define the morphism
      $(j_1, j_2) = ((f,g), \lambda (j_1,j_2).\lambda w.(F(j_1(g(w)))\circ G(j_2(f(w)))))
      : C \to A \times B$, where $(f,g) = \lambda w.(f(w),g(w))$ is the unique morphism
      from the cartesian structure of $\sets$, and thus, $(j_1, j_2)$ is unique.
      The condition on morphisms is also satisfied; see the formalization for the details.
    \end{itemize}
    All of the previous morphisms satisfy the usual diagram for
    cartesian products.    
  \end{itemize}
  Now we must define two functors $F : \dial \to \dial_!$ and $G :
  \dial_! \to \dial$ such that $G \dashv F$.  Take $F = !$ which was
  defined in the proof of
  Lemma~\ref{lemma:ddial_is_a_linear_category}.  Then define $G$ as
  follows:
  \begin{center}
    \begin{math}
      \begin{array}{rll}
        G(U, U \Rightarrow X^*, \alpha_!) & = & (U , X , \alpha)\\
        G((f , \lambda g.F^*(g(f(u)))) : (U , (U \Rightarrow X^*), \alpha_!) \to (V , (V \Rightarrow Y^*), \beta_!)) & = & (f, F) : (U , X, \alpha) \to (V , Y, \beta)\\
      \end{array}
    \end{math}
  \end{center}
  Here $F$ is obtained by the following reasoning:
  \[
  \begin{array}{llllll}
    \lambda v.(\lambda g.F^*(g(f(u))))(\lambda x.(v))
    & = & \lambda v.(\lambda g.F^*((\lambda x.(v))((f(u))))\\
    & = & \lambda v.(\lambda g.F^*((v)))\\
    & = & \lambda v.F(v)\\
    & = & F\\
  \end{array}
  \]
  Suppose $A = (U , X , \alpha)$ is an object of $\dial$. Then the
  natural transformation $\eta : \mathsf{Id}_{\dial} \to F;G$ is
  defined by $eta_{(U,X,\alpha)} : (U , X , \alpha) \to G(F(U , X , \alpha))$.
\end{proof}
