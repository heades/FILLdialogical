...
Game semantics has many applications in computer science particularly
in the study of the algebraic foundations of programming languages
with effects (e.g. states and continuations).  Blass gave a model of
linear logic \cite{Blass:1992} in game semantics, commonly known as
the Blass model, but Abramsky showed that this model does not define a
category, because associativity fails.  This was a major concern,
because both category theory and linear logic are highly used tools by
the game semanticist. After all the notion of a resource plays a
fundamental role in both theories. Melli\`es and Tabareau show that the
situation can be rectified by decomposing the modalities of linear
logic into what they call ``resource modalities'' resulting in a new
logic they call tensor logic \cite{Mellies:2010}.  They argue that
tensor logic is not merely a restriction of linear logic, but a
generalization due to the fact that linear logic can be embedded into
tensor logic in the same way that intuitionistic logic can be embedded
into linear logic.  Thus, they see tensor logic as being more
fundamental than linear logic. In this section we show that $\dial$
can be seen as a model of tensor logic.

Tensor logic can be seen as linear logic where the involutive negation
has been broken up into a what Melli\`es and Tabareau call a tensorial
negation (Definition~\ref{def:tensorial-negation}) and a resource
modality (Definition~\ref{def:resource-modality}).  Then using these
two notions the usual notion of polarities can be defined
\cite{Mellies:2010}.
\begin{definition}
  ...
  \]
\end{definition}
The most basic form of tensor logic is called multiplicative tensor
logic and only consists of tensor and a tensorial negation.  The model
of multiplicative tensor logic is called a dialogue category.
\begin{definition}
  \label{def:dialogue-cat}
  A \textbf{dialogue category} is a symmetric monoidal category
  equipped with a tensorial negation.
\end{definition}
We have already seen that $\dial$ is a symmetric monoidal category,
because it is a model of full intuitionistic linear logic, and thus,
if we can define a tensorial negation on $\dial$ then it can be seen
as a model of multiplicative tensor logic.
\begin{lemma}
  \label{lemma:dial-tensorial-model}
  The category $\dial$ is a model of multiplicative tensor logic.
\end{lemma}
\begin{proof}  
    We must show that $\dial$ is a dialogue category\footnote{This proof
      was formalized in the Agda proof assistant see the file
    ...
    \end{itemize}
\end{proof}
Extending a model of multiplicative tensor logic with an exponential
resource modality yields a model of full tensor logic.
\begin{definition}
...
A model of full tensor logic is defined to be a model of
multiplicative tensor logic with an exponential resource modality. We
now know that $\dial$ is a model of multiplicative tensor logic.  By
constructing the co-Kleisli category which consists of the
$!$-coalgebras as objects which is cartesian we can show that $\dial$
is a model of full tensor logic.  The adjunction with the co-Kleisli
...
\begin{proof}
  ...
  \dial \to \dial_!$.  Therefore, $\dial$ is a model of full tensor
  logic.
\end{proof}
...
we showed that $\dial$ is a model of full tensor logic.
