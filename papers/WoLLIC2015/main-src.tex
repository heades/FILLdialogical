...
\newcommand{\ddial}[0]{\mathsf{DDial_2}(\mathsf{Sets})}
...
showed that a particular dialectica category called
$\mathsf{DDial}_2(\mathsf{Sets})$ is a model of FILL where $!$ is
interpreted as a cofree comonad, see page 30 of \cite{dePaiva:1988}.
\begin{definition}
  \label{def:dial2sets}
  The category $\mathsf{DDial}_2(Sets)$ consists of 
  \begin{itemize}
  ...
  \item maps that are pairs $(f,F) : (U,X,\alpha) \to (V,Y,\beta)$
    ...
    \begin{itemize}
    \item For any $u \in U$ and $y \in Y$, $\alpha(u,F(u,y))$ implies $\beta(f(u),y)$.
    \end{itemize}
  \end{itemize}
\end{definition}
...
\cite{dePaiva:1988}.  The category $\mathsf{DDial}_2(\mathsf{Sets})$
defined above can be seen as an instantiation of $GC$ by setting $C$
to be the category $\mathsf{Sets}$ of sets and functions between them.
...
Benton gave a more fundamental view of linear categories by defining a
new categorical model of intuitionistic linear logic that is
equivalent to that of linear categories, but is much more simple.
\begin{definition}
  \label{def:LNL-model}
...
see page 15 of \cite{Benton:1994}.  We now have the following results:
\begin{theorem}[LNL Models are Linear Categories, page 16 of \cite{Benton:1994}]
  \label{thm:lnl_models_are_linear_categories}
  Every LNL model is a linear category.
\end{theorem}
\begin{proof}
  The proof is a matter of checking that each part of the
...
\begin{theorem}[Linear Categories are LNL Models, page 22 of \cite{Benton:1994}]
  \label{thm:linear_categories_are_lnl_models}
  Every linear category is a LNL model.
\end{theorem}
\begin{proof}
  Given a linear category we have a SMCC and so the difficulty of
...
\end{proof}
\noindent
Finally, these two results imply that linear categories are equivalent
to LNL models.

Next we show that the category $\mathsf{DDial}_2(\mathsf{Sets})$ is a
full version of a linear category. First, we extend the notion of a
linear category to be equipped with the necessary categorical
structure to model par and its unit.
\begin{definition}
  \label{def:linear-cat}
...
\begin{lemma}
  ...
  The category $\mathsf{DDial_2}(\mathsf{Sets})$ is a full linear category.
\end{lemma}
\begin{proof}
  We prove this by constructing each piece of a full linear category
  using the structure of $\mathsf{DDial_2}(\mathsf{Sets})$.
  ...
  $\ddial$ is symmetric monoidal closed:
  \begin{center}
    \begin{itemize}
    \item (Definition 7, page 43 of \cite{dePaiva:1988}). Suppose
      $A,B \in \obj{\ddial}$.  Then there are sets $X$, $Y$, $V$, and
      ...
      defined by $A \otimes B = (U \times X, (V \Rightarrow X) \times
      (U \Rightarrow Y),\alpha \otimes \beta)$, where $(- \Rightarrow
      -)$ is the internal hom of $\mathsf{Sets}$.  Lastly, we define
    ...
    \item (Definition 7, page 44 of \cite{dePaiva:1988}). Suppose
      ...
      $\diamond \subseteq 1 \times 1$.  Then we can define tensors
      unit by the object $(1,1,\diamond)$.

    \item (Definition 9, page 44 of \cite{dePaiva:1988}). Suppose
      $A,B \in \obj{\ddial}$.  Then there are sets $X$, $Y$, $V$, and
      $U$, and relations $\alpha \subseteq U \times X$ and
      $\beta \subseteq V \times Y$, such that, $A = (U,X,\alpha)$ and
      $B = (V,Y,\beta)$.  Then we define the internal hom of $\ddial$
      ...
      \ddial$ the internal hom $A \limp B \in \ddial$ exists.      
    \end{itemize}
  \end{center}
  Using the constructions above $\ddial$ is a SMCC by Proposition 24
  ...
  \begin{center}
    \begin{itemize}
    \item (Definition 15, on page 68 of \cite{dePaiva:1988}). The
      endofunctor $! : \ddial \to \ddial$ is defined as follows:
      \begin{itemize}
      \item Objects. Suppose $A = (U,X,\alpha) \in \obj{\ddial}$.
        Then we set $!A = (U,U \Rightarrow X, !\alpha)$, where $(u,f)
        \in T\alpha$ if and only if $(u,f(u)) \in \alpha$. 

      \item Morphisms. Suppose $A = (U,X,\alpha) \in \obj{\ddial}$,
        $B = (V,Y,\beta) \in \obj{\ddial}$, and
        $(f,F) : A \to B \in \mor{\ddial}$.  Then we define 
        $!(f,F) = (f,!F) : !A \to !B$, where $!F(u,g) = \lambda
        x.F(x,g(f(x))) : U \times (V \Rightarrow Y) \to U \Rightarrow X$.
      \end{itemize}

    \item (Proposition 39, page 68 of \cite{dePaiva:1988}). The
      endofunctor $!$ defined above is the functor part of the comonad
      $(!,\epsilon, \delta)$.  Suppose
      $A = (U,X,\alpha) \in \obj{\ddial}$. Then the co-unit
      $\epsilon : !A \to A$ is defined by $\epsilon =
      (\mathsf{id}_U,F)$ where $F(u,x) = \lambda y.x : U \times X \to
      U \Rightarrow X$.  Furthermore, the co-multiplication $\delta_A
      : !A \to !!A$ is defined by $\delta_A = (\mathsf{id}_U,F')$
      where $F'(u,f) = \lambda u'.f(u,u') : U \times (U \Rightarrow (U
      \Rightarrow X)) \to U \Rightarrow X$.
    ...
    \item The following diagrams commute:
      \begin{center}
        ...
        \end{math}
      \end{center}
      We show the left most diagram commutes first.  Suppose
      $A = (U,X,\alpha) \in \obj{\ddial}$. Then by the definitions
      given above we can see that $\delta_A = (\id_U,F')$, $!\delta_A = (\id_U,!F')$ and
      $\delta_{!A} = (\id_U,F'')$ where $F'(u,f) = \lambda u'.f(u,u') : U \times (U \Rightarrow (U \Rightarrow X)) \to U \Rightarrow X$, $!F'(u,f) = \lambda
      x.\lambda u'.g(x)(x,u')
      : U \times (U \Rightarrow (U \Rightarrow (U \Rightarrow X))) \to U
      \Rightarrow (U \Rightarrow X)$ and $F''(u,f) = \lambda
      u'.f(u,u')$ with the same type as $F'$.  First, we compute $\delta_A;\delta_{!A} = (\id_U,F;F')$, where
      \begin{center}
        \begin{math}
          \begin{array}{lll}
            F';F''
            & = & \lambda x.\lambda y.F'(x,F''(x,y))\\
            & = & \lambda x.\lambda y.F'(x,\lambda u'.y(x,u'))\\
            & = & \lambda x.\lambda y.\lambda u''.(\lambda u'.y(x,u'))(x,u'')\\
            & = & \lambda x.\lambda y.\lambda u''.y(x,x,u'')\\
          \end{array}
        \end{math}
      \end{center}
      and now we compute $\delta_A;!\delta_A = (\id_U,F';!F')$, where 
      \begin{center}
        \begin{math}
          \begin{array}{lll}
            F';!F' 
            & = & \lambda x.\lambda y.F'(x,!F'(x,y))\\
            & = & \lambda x.\lambda y.F'(x,\lambda x'.\lambda u'.y(x')(x',u'))\\
            & = & \lambda x.\lambda y.\lambda u''.(\lambda x'.\lambda u'.y(x')(x',u'))(x,u'')\\
            & = & \lambda x.\lambda y.\lambda u''.y(x)(x,u'')\\
            & = & \lambda x.\lambda y.\lambda u''.y(x,x,u'')\\
          \end{array}
        \end{math}
      ...
      Now we prove that the second diagram commutes, but we break it into two.
      First, recall that $\epsilon_A = (\id_U,F)$ and $!\epsilon_A = (\id_U,!F)$ where 
      $F(u,x) = \lambda y.x$ and $!F(u,g) = \lambda x.\lambda y.g(x)$.  Then we can 
      define $\delta_A;!\epsilon_A = (\id_U,F';!F)$ where
      \begin{center}
        \begin{math}
          \begin{array}{lll}
            F';!F 
            & = & \lambda x.\lambda y.F'(x,!F(x,y))\\
            & = & \lambda x.\lambda y.\lambda u'.!F(x,y)(x,u')\\
            & = & \lambda x.\lambda y.\lambda u'.(\lambda x'.\lambda y'.y(x'))(x,u')\\
            & = & \lambda x.\lambda y.\lambda u'.y(x)\\
          \end{array}
        \end{math}
      \end{center}
      and we can define $\delta_A;\epsilon_{!A} = (\id_U,F';F)$ where
      \begin{center}
        \begin{math}
          \begin{array}{lll}
            F';F 
            & = & \lambda x.\lambda y.F'(x,F(x,y))\\
            & = & \lambda x.\lambda y.F'(x,\lambda y'.y)\\
            & = & \lambda x.\lambda y.\lambda u'.(\lambda y'.y)(x,u')\\
            & = & \lambda x.\lambda y.\lambda u'.y\\
          \end{array}
        \end{math}
      \end{center}
      Note that both of the above morphisms should end up being equivalent to $\pi_2$, but they clearly are not.
      \todo{Here is the problem.}      
    \end{itemize}
  \end{center}
\end{proof}
