    ...
    \item The monoidal natural transformation
      ...
      \begin{center}
          \begin{mathpar}
            ...
            \and
            \bfig
            \hSquares/->`->`->``->`->`->/[(!A \otimes !B) \otimes !C`!(A \otimes B) \otimes !C`!((A \otimes B) \otimes C)`!A \otimes (!B \otimes !C)`!A \otimes !(B \otimes C)`!(A \otimes (B \otimes C));m_{A,B} \otimes \id_{!C}`m_{A \otimes B,C}`\alpha_{!A,!B,!C}``\id_{!A} \otimes m_{B,C}`!\alpha_{A,B,C}`m_{A,B \otimes C}]
            \place(1300,250)[\text{C}]
            \efig
          \end{mathpar}          
      \end{center}
      We first prove that diagram A commutes, and then diagrams B will
      commute using similar reasoning.
    \end{itemize}    
  \end{center}
