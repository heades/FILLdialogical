...
\begin{abstract}
  ...
  suggested. Additionally, we show that the categorical model of FILL
  in a free dialectica category is also a LNL model of Benton.
\end{abstract}
...
free dialectica category, is also a LNL model of Benton
...
Benton gave a more well balanced view of linear categories called LNL
models.
\begin{definition}
  \label{def:LNL-model}
...
\begin{definition}
  ...
  A \textbf{full linear category}, $\mathcal{L}$, consists of:
  \begin{center}
    \begin{itemize}
    \item A linear category $(\mathcal{L}, \top, \otimes, \limp,e_A,d_A)$,
    \item a symmetric monoidal category $(\mathcal{L}, \perp, \parr)$, and
...
\begin{definition}
  ...
  \begin{itemize}
  \item A LNL model $(\cat{L}, \cat{C},F,G)$,
  \item a symmetric monoidal category $(\mathcal{L}, \perp, \parr)$, and
  \item distribution natural transformations $\mathsf{dist}_1 : A \otimes (B
      \parr C) \to (A \otimes B) \parr C$ and $\mathsf{dist}_2 : (A \parr B)
      \otimes C \to A \parr (B \otimes C)$ in $\cat{L}$.
  \end{itemize}
\end{definition}

\noindent
Our main result is to first prove that $\dial$ is a full linear
category, and then using the proof by Benton that linear categories
are LNL models we obtain that $\dial$ is a full LNL model, but in
order for this to work we need to know that $\dial$ has a symmetric
monoidal comonad $(!, \epsilon, \delta, m_{A,B}, m_I)$.  However, at
the time of de Paiva's thesis it was not known that the comonad
modeling the of-course exponential needed to be symmetric monoidal.
We were able to show that the maps $m_{A,B}$ and $m_I$ exist in the
more general setting of dialectica categories, and thus, such maps
exist in $\dial$. We can now show our main result of this section.
\begin{lemma}
...
\begin{proof}
  \begin{paper}
    ...
    structure of $\dial$ can be extracted from de Paiva's thesis
    \cite{dePaiva:1988}.  However, we had to extend her definitions to
    show that the free comonad $(!A,\delta,\epsilon)$ is monoidal,
...
\begin{report}
  ...
  \item The map $d_A$ must be a monoidal natural transformation.  This
    ...
    $F_\otimes;F_{iso};F_\otimes = \Theta;F_2$:
    \begin{center}
      \begin{math}
        \begin{array}{lll}
...
\to \neg\neg A$ that is a refinement of the isomorphism of Linear
Logic, that $\dial$ would be a model of tensorial logic. After all in
$\dial$ we do have a natural transformation of the shape described,
taking an object $(U,X, \alpha)$ to $(X, U, \neg \alpha)$ and then to
$(U, X, \neg\neg \alpha)$ which is "almost" an isomorphism: we use
identities in U and X, but unless the predicate $\alpha$ itself is
double-negated, we have a morphism $\alpha \to \neg\neg \alpha$, but
not a converse one. But we have not had the time to check whether the
other structure of tensorial logic is present and hence we leave this
to future work.
