...
\begin{abstract}
  ...
  suggested. Additionally, we show that this new formalization can be
  modeled by a new form of dialectica category called order-enriched
  dialectica category, and discuss future work giving FILL a semantics
  in terms of Lorenzen games.
\end{abstract}
...
$\Delta$ are multisets of formulas.  Sequents of this form are read as
``the conjunction of the formulas in $\Gamma$ imply the disjunction of
the formulas in $\Delta$''.  For a brief, but more complete history of
logics with multiple conclusions see the introduction to
\cite{dePaiva:2005}.
...
enforcing intuitionism.  Maehara showed that one could simply place
the cardinality restriction on the premise of the implication right
rule, and leave all of the other rules of LK unrestricted.  This
restriction is sometimes called the Dragalin restriction.  The
classical implication right rule has the form:
\begin{center}
  \begin{math}
    $$\mprset{flushleft}
...
(Section~\ref{sec:full_intuitionistic_linear_logic_(fill)}) and prove
cut-elimination (Section~\ref{sec:cut-elimination}).  In addition, we
show that this new formalization can be modeled by a new form of
dialectica category called order-enriched dialectica category
...
Linear Logic (FILL).  We first give the syntax of formulas, patterns,
terms, and contexts.  Following the syntax we define several
meta-functions that will be used when defining the inference rules of
the logic.  
...
The formulas of FILL are standard where we denote the unit of tensor
as $\top$ and the unit of par as $\perp$. Patterns are used to
...
Definition~\ref{def:FILL-eq}.  The role each term plays in the overall
logic will become clear after we introduce the inference rules.

Left contexts are multisets of formulas labeled with a variable, and
right contexts are multisets of formulas labeled with a term.  We will
often write $[[L1 | L2]]$ as syntactic sugar for $[[L1,L2]]$.  The
former should be read as ``$[[L1]]$ or $[[L2]]$.'' This will help
readability of the sequent we will introduce below.  

We denote the usual capture-avoiding substitution by $[[ [t/x]t']]$,
and its straightforward extension to right contexts as $[[ [t/x]L]]$.
% \begin{definition}
%   \label{def:delta-sub}
...
The following definition defines the inference rules of FILL.
\begin{definition}
  \label{def:infr}
...
variables in $[[x (+) -]]$ and $[[- (+) y]]$ in a term if and only if
those pattern variables are free in the term.  This over comes the
...
\section{Order-Enriched Dialectica Categories}
\label{sec:order-enriched_dialectica_categories}
\cite{dePaiva:2015}
% section order-enriched_dialectica_categories (end)
