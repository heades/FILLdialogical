...
\author{Harley Eades III\inst{1} \and Valeria de Paiva\inst{2}}
\institute{
    Computer and Information Sciences\\ Georgia Regents University\\ Augusta, GA
    \and Nuance Communications\\ Sunnyvale, CA}
\date{}
...
\begin{abstract}
  ...
  also a LNL model of Benton and a full tensor model of Melli\`es' and
  Tabareau's tensorial logic.  Lastly, we give a double-negation
  translation of linear logic into FILL that explicitly uses par in
  addition to tensor.
\end{abstract}
...
translation of multi-conclusion linear logic into FILL
(Seciton~\ref{subsec:a_negative_translation_of_linear_logic_into_fill_using_tensorial_negation}).
...
possible to prove cut-elimination of the term assignment formalization
of FILL relative to Bra\"uner and de Paiva's dependency tracking
system by erasing the terms on conclusions and then tracking which
variable is free in which conclusion.  However, as we stated above a
direct proof is more desirable than a relative one.
...
\begin{proof}
  ...
  left-to-right rewrite rules. \begin{paper} Due to space limitations
    we only show one of the most interesting cases, but for the entire
    proof see the companion report \cite{Eades:2015}.\end{paper}
  \begin{itemize}
    \begin{report}      
...
%%   Suppose $(\cat{C}, \otimes, \top, \limp, \perp)$ is a SMCC with an
%%   object $\perp$ dual to $\top$. Then $\cat{C}$ is a model of
%%   multiplicative tensorial logic.
%% \end{lemma}
%% \begin{proof}  
%%   Suppose $(\cat{C}, \otimes, \top, \limp, \perp)$ is a SMCC with an
%%   object $\perp$ dual to $\top$.  It suffices to show that that we can
%%   define a tensorial negation.  Define the negation functor by $\lnot
%%   A = A \limp \perp$.  Finally, by Lemma~\ref{lemma:monoidal-fact1}
%%   $\lnot A$ is a tensorial negation.  Therefore, $\cat{C}$ is a
%%   dialogue category.
%% \end{proof}
\begin{lemma}
...
\begin{proof}
  ...
  the unit of par, and thus, by Lemma~\ref{lemma:monoidal-fact1} has a
  tensorial negation\footnote{We give a full proof in the
    formalization see the file
...
are LNL models by Benton. In Section~\ref{sec:tensor_logic} we showed
that $\dial$ is a model of full tensorial logic.  Finally, we gave a
double-negation translation of multi-conclusion linear logic into FILL
that explicitly uses both tensor and par.
