...
[h{let-pat z (- (+) y) t'}/w]L2]]$.  This follows by a similar
argument as the previous cases using let-pat distribution
(Lemma~\ref{lemma:let-pat_distribution}).
...
use the $\FILLdrulename{EtaLet}$ rule to obtain $[[let w be x (x) y in
L = L]]$.  Similarly, we know $[[let w be x (x) y in [t/x]t1 = [let w
be x (x) y in t/x]{let w be x (x) y in t1}]]$ by let-distribution
(Lemma~\ref{lemma:let_distribution}), and then by using
$\FILLdrulename{EtaLet}$ and the fact that $[[x,y nin FV(t)]]$ we know
$[[ [let w be x (x) y in t/x]{let w be x (x) y in t1} = [t/x]{let w be
  x (x) y in t1}]]$.  This argument can be repeated for any term in
$[[let w be x (x) y in [t/x]L1]]$, thus, $[[ [t/x](let w be y (x) z in
L1) = let w be x (x) y in [t/x]L1]]$.
...
case analysis on whether or not $[[L1]]$ is empty, let-pat
distribution (Lemma~\ref{lemma:let-pat_distribution}), and the fact
that $[[y nin FV(t)]]$.
...
\begin{center}
  ...
  \end{math}
\end{center}
{\color{red}{There is a problem here.}}
% subsubsection left_introduction_of_tensor_unit (end)
